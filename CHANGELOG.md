...
- 追忆 AI 照片成片新增外公、周雅琴与狗狗小满三组独立授权素材及对应视频，连同既有母亲素材形成四套互不复用的照片、成片与文件标识；人类与宠物纪念轨道继续隔离。
- 生成反馈升级为五阶段非线性进度、原图扫描光效、阶段细节提示和完成淡入；播放器按各素材真实画幅与时长展示，结果持续标注 AI 合成、授权用途和非真实拍摄属性。
- 旧版本本地快照会在保留已有纪念内容的前提下补入新增纪念空间；狗狗影像修复同步使用对应宠物素材。
- 应用版本提升至 `2.1.1`（`versionCode 6`）。

### 质量

- `testDebugUnitTest`：39 套件 / 249 用例 / 0 失败 / 0 错误；`lintDebug` 与 `assembleRelease` 通过。
- Pixel 7 API 36 已实际验证周雅琴、小满、外公三条照片成片链路，生成动画、真实画幅、时长与视频播放均正常。
