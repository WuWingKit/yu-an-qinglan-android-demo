...
- 公开 V2.1.0 源码归档文件与 APK 安装包的可信时间戳认证证书、双语预览图和证书 PDF 完整性哈希。
- 新增可信时间戳认证与著作权声明，说明证据用途、验证方式、与计算机软件著作权登记证书的区别，以及仓库专有 License 继续适用的边界。

## [2.1.0] - 2026-09-07

### 修复与调整

- 人间树洞与生灵树洞分别更新为 15 条指定文案，继续保持双池数据、ID 与分类隔离。
- 宠物纪念空间的影像修复预览、结果预览及本机保存统一使用宠物肖像，不再误用人物素材；人类纪念空间维持原人物修复素材。
- 重新制作首页“山水有归处”和“花开四季”轮播图，移除素材内嵌的留白区域，保持原尺寸并全幅展示。
- 优化树洞轻回应动画：按钮颜色与图标缩放平滑过渡，叶片增加自然收放，花朵增加克制光晕；既有低动态降级、点击节流和无计数原则保持不变。
- 版权及授权详情页沿用仓库专有 License，补充权利归属、许可边界、禁止行为、GitHub 平台权限、第三方内容与责任；授权情况仅说明软件使用权已授权给李芸凤，并保留软件使用授权书查看入口。
- 应用版本提升至 `2.1.0`（`versionCode 5`）。

### 质量

- 增加人宠影像修复轨道选择回归测试；树洞内容门禁固定校验每池 15 条。
- `testDebugUnitTest`：38 套件 / 247 用例 / 0 失败 / 0 错误；`lintDebug` 与 `assembleRelease` 通过；Pixel 7 API 36 上 `connectedDebugAndroidTest` 15 / 15 通过。
