...
/** 供 androidTest Compose 测试使用的公共设置仓库假实现。 */
...
/** 供 androidTest Compose 测试使用的个人中心本地仓库假实现。 */
class FakeProfileLocalStore : ProfileLocalStore {
    ...
    override suspend fun verifyPassword(candidate: String): Boolean {
        val hash = passwordHash ?: sha("12345678")
        return sha(candidate) == hash
    }
...
/** 供 androidTest Compose 测试使用的文件搬运假实现。 */
class FakeMediaImporter : ProfileMediaImporter {
    var importImageFail = false
    val importedImages = mutableListOf<String>()
    val deletedUris = mutableListOf<String>()

    override suspend fun importImageToPrivate(sourceUri: String): String? {
        if (importImageFail) return null
        val uri = "file:///data/user/0/com.yuanqinglan.app/files/yuanqinglan/images/avatar_${importedImages.size}.jpg"
        importedImages.add(uri)
        return uri
    }

    override suspend fun importAudioToPrivate(sourceUri: String): String? =
        "file:///data/user/0/com.yuanqinglan.app/files/yuanqinglan/audio/test.m4a"
