...
“渝安青澜”V2.1.0 源码归档文件与 APK 安装包已于 **2026年09月08日**分别取得联合信任时间戳服务中心签发的可信时间戳认证证书。证书编号、认证时间、认证码、PDF 原件、双语图片预览及验证说明见 [可信时间戳认证与著作权声明](docs/COPYRIGHT_EVIDENCE.md)。

可信时间戳用于证明其对应电子数据自申请时间起已经存在且内容保持完整、未被篡改；其性质不同于国家版权主管部门颁发的计算机软件著作权登记证书。仓库公开认证材料不构成任何新增许可，使用边界仍以本仓库 [专有软件许可协议](LICENSE) 及权利人签发的特定书面授权为准。
